document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleTorch = async () => {
        ...
        const torchStrategies = [
            { advanced: [{ torch: nextTorchState }] },
            { advanced: [{ fillLightMode: nextTorchState ? 'flash' : 'off' }] },
        ];

        let applied = false;
        for (const constraints of torchStrategies) {
            applied = await applyVideoConstraints(constraints);
            if (applied) {
                break;
            }
        }
    ...
    const getPreferredCameraDevice = (devices = []) => {
        ...
        const rankedDevices = devices
            .map((device) => {
                ...
                if (/macro/.test(label)) score += 120;
                if (/telephoto|closeup/.test(label)) score += 95;
                if (/back|rear|environment|tr[aá]s|posterior/.test(label)) score += 70;
                if (/wide|ultra/.test(label)) score -= 10;
                if (/front|user|selfie|frontal/.test(label)) score -= 100;
    ...
    const applyAutoMacroPreset = async () => {
        ...
        if (typeof capabilities.zoom !== 'undefined') {
            const minZoom = typeof capabilities.zoom.min === 'number' ? capabilities.zoom.min : 1;
            const maxZoom = typeof capabilities.zoom.max === 'number' ? capabilities.zoom.max : 3;
            const autoMacroZoom = Math.min(maxZoom, Math.max(minZoom, isMobileDevice ? 2.6 : 2));
        ...
        if (appliedAnySetting) {
            ...
            zoomEnabled = advanced.some((setting) => Object.prototype.hasOwnProperty.call(setting, 'zoom'));
            setToolActive(btnZoom, zoomEnabled);
    ...
    const startScanner = async () => {
        ...
        const config = {
            ...
            qrbox: (viewfinderWidth, viewfinderHeight) => {
                const width = Math.min(viewfinderWidth * 0.82, 360);
                const height = Math.min(Math.max(width * 0.62, 150), viewfinderHeight * 0.52);
                return {
        ...
        const preferredDevice = getPreferredCameraDevice(devices);
        const cameraCandidates = [
            preferredDevice?.id,
            { facingMode: { exact: 'environment' } },
            { facingMode: 'environment' },
            devices.find((device) => /back|rear|environment|tr[aá]s|posterior/i.test(device.label || ''))?.id,
            devices[0]?.id,
            { facingMode: 'user' },
        ].filter(Boolean);
